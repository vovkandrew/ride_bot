...
@Component
public class EditTripSetDropOffPoint extends UpdateHandler {
    private final TripService tripService;

    public EditTripSetDropOffPoint(TripService tripService) {
        this.tripService = tripService;
    }

    @Override
    public boolean isApplicable(Optional<Phase> phaseOptional, Update update) {
        return isUpdateContainsHandler(update, DRIVER_TRIP_EDITING_DROPOFF_POINT) || super.isApplicable(phaseOptional, update);
    }
    ...
    public void handle(UserPhase userPhase, Update update) throws TelegramApiException {
        ...
        if (isUserInputMatchesPattern(userInput, getGeneralMessagePatternWithParameterizedStartLimit(10))) {
            ...
            tripService.updateTripDropOffPoint(trip, userInput);
